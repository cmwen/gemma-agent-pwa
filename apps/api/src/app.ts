...
export function createApiApp(workspaces: WorkspaceRegistry) {
  ...
  app.use(
    ...
    cors({
      origin: (origin) =>
        isAllowedCorsOrigin(origin, allowedCorsOrigins) ? origin : null,
  ...
  app.get("/api/models", async (context) => {
    return context.json(await listAvailableModels());
  });
...
function parseForwardedProps(value: unknown): {
  ...
  return {
    ...(record.config
      ? { config: parseForwardedRuntimeConfig(record.config) }
      : {}),
...
function parseForwardedRuntimeConfig(value: unknown): PartialChatRuntimeConfig {
  const parsed = partialChatRuntimeConfigSchema.parse(value);
  const raw = value as Record<string, unknown>;
  const config = Object.fromEntries(
    Object.entries(parsed).filter(([key]) => key in raw)
  ) as PartialChatRuntimeConfig;

  if (config.provider !== undefined) {
    config.provider = requireConfiguredProvider(config.provider);
  }
